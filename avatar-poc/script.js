        ...
        if (prescriptions.length === 0) {
            ...
            listBody.innerHTML = prescriptions.map(rx => `
                <div class="rx-card" style="background:#0f172a; padding:15px; margin-bottom:10px; border-radius:10px; border:1px solid #334155;">
                    ...
                    <div style="font-size:0.9rem; color:#94a3b8; margin-bottom:5px;">${rx.notes}</div>
...
function openBookingModal(specialty = 'General Physician') {
    ...
    sortedDoctors.forEach((doc, index) => {
        ...
        html += `
            <div class="doctor-card ${isRecommended ? 'recommended' : ''}" id="doctor-${index}" onclick="selectDoctor(${index}, ${doc.id}, '${doc.name}', 'Today 3:00 PM')">
                <div class="doctor-avatar">${initials}</div>
                <div class="doctor-info">
...
function confirmBooking() {
    ...
    // Filter only meaningful messages (User and Model)
    ...
    StorageService.saveAppointment({
        patientName: 'John Doe', // Hardcoded current user
        specialty: currentSpecialty,
        doctorName: selectedDoctor.name,
        doctorId: selectedDoctor.id, // Ensure we save ID for filtering
        time: selectedDoctor.time,
        chatHistory: chatSummary // Save the chat log!
    });
    ...
    < div class="booking-success" >
            ...
            <div class="booking-details">
                ...
                <p><strong>Specialty:</strong> ${currentSpecialty}</p>
